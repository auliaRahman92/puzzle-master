...
class Puzzle {
  ...
  */
  ...
  scale() {
    ...
    // Modify the scale calculation for 12 pieces
    const maxWidth = (this.nbPieces === 12) ? 
        0.75 * this.contWidth : // 75% for 12 pieces
        0.95 * this.contWidth;  // 95% for other piece counts
    
    const maxHeight = (this.nbPieces === 12) ? 
        0.75 * this.contHeight : // 75% for 12 pieces
        0.95 * this.contHeight;  // 95% for other piece counts
    ...
    // Scale pieces with additional spacing for 12 pieces
    this.scalex = (this.nbPieces === 12) ?
        (this.gameWidth / this.nx) * 0.9 :  // 90% scale for 12 pieces
        this.gameWidth / this.nx;            // normal scale for others
        
    this.scaley = (this.nbPieces === 12) ?
        (this.gameHeight / this.ny) * 0.9 :  // 90% scale for 12 pieces
        this.gameHeight / this.ny;           // normal scale for others

    // Recalculate offset for centering
    this.offsx = (this.contWidth - this.gameWidth) / 2;
    this.offsy = (this.contHeight - this.gameHeight) / 2;

    // Adjust connection distance for 12 pieces
    this.dConnect = (this.nbPieces === 12) ?
        mmax(15, mmin(this.scalex, this.scaley) / 8) :  // larger connection distance for 12 pieces
        mmax(10, mmin(this.scalex, this.scaley) / 10);  // normal connection distance
...
let menu = (function () {
  ...
  document.addEventListener('click', (event) => {
    // Check if click is outside menu and menu is opened
    if (menu.opened && !event.target.closest('#menu')) {
        menu.close();
    }
});
